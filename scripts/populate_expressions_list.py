...
output_folder = path.join(
    path.dirname(__file__), "..", "docs/user_manual/expressions/expression_help"
)
# images file location
img_list_file = path.join(
    path.dirname(__file__),
    "..",
    "docs/user_manual/expressions/expression_help/img/images_list.json",
)
algorithm_folder = path.join(
    path.dirname(__file__), "..", "docs/user_manual/processing_algs"
)
...
def format_variant(function_dict, f_name):
    ...
    if "arguments" in function_dict:
        for arg in function_dict["arguments"]:
            ...
            if "syntaxOnly" in arg:
                ...
                arg_description_list.append(
                    f"**{arg['arg']}** - {sphynxify_html(arg['description'],9)}"
                )
            else:
                arg_syntax_list.append(arg_text)
                arg_description_list.append(
    ...
    elif f_name.startswith("$") or (
        "type" in function_dict and function_dict["type"] == "value"
    ):
        syntax += f"{f_name}\n"
        arguments = ""
    else:
...
def create_function_anchor(func, group):
    ...
    """
    return "expression_function_{}_{}".format(
        group, functions[func]["filename"]
    ).replace(" ", "_")
    ...
    if anchor["img"]:
        ...
        if glob.glob(temp_path):
            ...
            if glob.glob(temp_path, recursive=True):
                ...
                print(
                    f'No image named {anchor["img"]} in expression and algorithms folders. '
                    "Please add one."
                )
    else:
...
for file in filenames:
    ...
    if data["type"] == "group":
        ...
        print(
            f'{data["name"]} is not a group, a function, a value or an operator. It is a {data["type"]}.'
        )
    ...
    if "groups" in function:
        for g_name in function["groups"]:
            if g_name not in ("deprecated"):
                ...
                groups[g_name]["func_list"][function["name"]] = create_function_anchor(
                    f_name, g_name
                )
            else:
...
for g_name in groups:
    ...
    func_list = {
        key: val
        for key, val in sorted(func_list.items(), key=lambda ele: ele[0].strip("$"))
    }
    output_group_file = path.join(output_folder, g_name.replace(" ", "_") + ".rst")
    with open(output_group_file, "w", newline="\n") as f:
        f.write(
            f":orphan:\n\n"
        ...
        for f_name in func_list:
            ...
            if func_list[f_name] in img_data.keys():
                if find_img_path(img_data[func_list[f_name]]):
                    f.write(
                        f".. figure:: {find_img_path(img_data[func_list[f_name]])}\n"
                        f"   :align: center\n\n"
