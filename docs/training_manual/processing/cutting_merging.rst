...
To calculate the bounding box , we can use the :guilabel:`Polygon from layer extent` algorithm

.. figure:: img/cutting_merging/bbox.png

To buffer it, we use the :guilabel:`Fixed distance buffer` algorithm, with the following parameter values.
...
by running the :guilabel:`Polygon from layer extent` algorithm on it. We could have buffered
...
to obtain, we can crop both of the raster layers, using the :guilabel:`Clip raster with polygon` algorithm.
...
A slope layer can be computed with the :guilabel:`Slope, Aspect, Curvature` algorithm,
...
The slope produced by the :guilabel:`Slope, Aspect, Curvature` algorithm can be expressed
