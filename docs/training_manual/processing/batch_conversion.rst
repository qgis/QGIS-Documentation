...
your output files, enter a filename (for instance, :file:`reprojected.shp`).

Now, when you click :guilabel:`OK` on the file selection dialog, the file does not
...
pattern. In this case, we are going to select the :guilabel:`Fill with parameter value`
...
Click on :guilabel:`OK` and the batch process will be run. If everything went fine,
