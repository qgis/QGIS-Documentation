...
.. tip::
   **Restoring toolbars** 
   ...
   If a toolbar disappears under Windows, which seems to be a problem in qgis from time to time, you have to remove \HKEY_CURRENT_USER\Software\QuantumGIS\qgis\UI     
   \state in the registry. When you restart qgis, the key is written again with the default state, and all toolbars are visible again. 
