...
Now that our PostgreSQL database is now geospatially enabled, thanks to PostGIS.
...
First we use the psql command to find functions relating to point:

::

  \df *point*

Here is an interesting one: :kbd:`st_pointfromtext`
...
In addition to the PostGIS functions, the extension contains  collection of 
