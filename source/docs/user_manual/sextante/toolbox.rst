...
plugins (more specifically, from the *fTools* vector library shipped along
...
By default, SEXTANTE will show a description of the CRS of each layer 
along with its name, making it easy to select layers that share the same
CRS to be used as input layers. If you do not want to see this additional 
information, you can disable this functionality in the SEXTANTE config
dialog, unchecking the "Show CRS***".
...
will produce wrong results, such as empty ayer due to input layers not
overlapping.
