...
options(
    ...
    virtualenv=Bunch(
        ...
        packages_to_install = [
            ...
            # NOTE: on windows download the tx.exe from http://support.transifex.com/customer/portal/articles/998120-client-on-windows
            # and put it either in your PATH or next to pavement.py
            # you also need a .transifexrc file (credentials file) into your home directory
            ],
...
def setup():
    ...
    if currentos == 'win32':
        ...
        # in this setup, you either have tx.exe next to pavement.py, OR it is available in your PATH
        options.sphinxintl.transifexclientbin = 'tx.exe'
...
def sphinxintl(options):
    ...
    if options.setup.lang=='en':
        print 'Building "en" language, no need to get .po files from transifex'
    else:
